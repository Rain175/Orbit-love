...
export default function PetTab({ room, onLocalUpdate }) {
  const [renaming, setRenaming] = useState(false);
  const [draftName, setDraftName] = useState(room.pet_name);
  const [burst, setBurst] = useState([]);
  const petRef = useRef(null);

  useEffect(() => setDraftName(room.pet_name), [room.pet_name]);
  ...
  async function saveName() {
    ...
    if (!name || name === room.pet_name) return;
    ...
    <div className="grid gap-6 md:grid-cols-[1.1fr_1fr]">
      <div className="card-cute relative flex flex-col items-center justify-center overflow-hidden p-8">
        ...
        <div className="relative mt-6 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">is feeling {mood.label}</p>
        </div>
      </div>
